        ...
        <tr style={{ zIndex: "9999", backgroundColor: `${isMove[column.id] || column?.isAdded ? "rgba(var(--vz-primary-rgb), 0.1)" : ""}` }}
            ...
            <td className={`bg-${isForeignColumn ? "soft-light" : "transparent"}`}>
                ...
                <Input
                    ...
                    disabled={(isForeignColumn === true && column.realColumnId == null)}
            ...
            <td className={`text-center bg-${isForeignColumn ? "soft-light" : "transparent"}`} >
                {isMove[column.id]
                    ? moveAction()
                    : <>
                        <div id={`column-popconfirm-${column.id}`} className={`btn btn-sm btn-soft-danger text-danger btn-hover`}>
                            <i className={`ri-delete-bin-5-fill fs-14 text-dark`}></i>
                        </div>
                        <PopConfirm
