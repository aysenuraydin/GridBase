...
export const useDeleteTable = () => {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ id, hard = true }: { id: number; hard?: boolean }) =>
            deleteTable(id, hard),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ["gridbase", "tables", "all"] });
            qc.invalidateQueries({ queryKey: ["gridbase", "tables", "one"] });
...
export const useDeleteColumn = (tableName: string) => {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ columnName, hard = true }: { columnName: string; hard?: boolean }) =>
...
export const useGetTableAccess = <T = any>(
    tableId: number, 
) =>
    useQuery<T>({
        queryKey: ['tableAccess', tableId],
        queryFn: () =>  getTableAccess(tableId),
        enabled: !!tableId,
    });
...
export const gridbaseSchemaKey = (tableName: string) =>
    ["gridbase", tableName, "schema"] as const;

export const useGridbaseSchema = (tableName: string) =>
    useQuery({
        queryKey: gridbaseSchemaKey(tableName),
        queryFn: () => getTableSchema(tableName),
        enabled: !!tableName,
    });

// queryKey üreticisi (gridbaseSchemaKey kalıbında)
export const gridbaseValidationKey = (tableName: string, columnName: string) =>
    ["gridbase", "validation", tableName, columnName];

// GET hook
export const useGetColumnValidation = (
    tableName: string,
    columnName: string,
    enabled: boolean = true
) =>
    useQuery({
        queryKey: gridbaseValidationKey(tableName, columnName),
        queryFn: () => getColumnValidation(tableName, columnName),
        enabled: enabled && !!tableName && !!columnName,
    });

// PUT (mutation) hook
export const useSetColumnValidation = (tableName: string) => {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (payload: {
            columnName: string;
            type: string;
            rules: Array<{ rule: string; isActive?: boolean; value?: string | null; message?: string | null }>;
        }) => {
            const { columnName, ...body } = payload;
            return setColumnValidation(tableName, columnName, body);
        },
        onSuccess: (_d, vars) => {
            qc.invalidateQueries({ queryKey: gridbaseValidationKey(tableName, vars.columnName) });
        },
    });
};
