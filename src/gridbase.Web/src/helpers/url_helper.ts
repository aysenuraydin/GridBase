...
// --- GRIDBASE: schema (Console için kolon meta + template) ---
export const GRIDBASE_SCHEMA = (tableName: string) => `/gridbase/${tableName}/schema`;
export const GRIDBASE_COLUMN_VALIDATION = (tableName: string, columnName: string) =>
    `/gridbase/${tableName}/columns/${columnName}/validation`;
...
export const PROJECT_QUOTA = "/projects/quota";  
export const PROJECT_BY_ID = (id: number | string) => `/projects/${id}`; 

// API Keys
export const PROJECT_KEYS = (projectId: number) => `/projects/${projectId}/keys`;
export const PROJECT_KEY_BY_ID = (projectId: number, keyId: number) =>
    `/projects/${projectId}/keys/${keyId}`;

// Dashboard 
export const PROJECT_OVERVIEW = (id: number) => `/projects/${id}/overview`;

// CORS
export const PROJECT_CORS = (projectId: number) => `/projects/${projectId}/cors`;
export const PROJECT_CORS_BY_ID = (projectId: number, originId: number) =>
    `/projects/${projectId}/cors/${originId}`;
