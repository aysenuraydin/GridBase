...
export interface CreatedApiKey {
    id: number;
    keyType: ApiKeyType;
    rawKey: string;       // ← BIR KEZ
    keyPrefix: string;
    name?: string | null;
    createdAt: string;
}

export const getProjectKeys = (projectId: number): Promise<ApiKeyListItem[]> =>
    api.get(url.PROJECT_KEYS(projectId));

export const createProjectKey = (
    projectId: number,
    body: { keyType: ApiKeyType; name?: string }
): Promise<CreatedApiKey> =>
    api.create(url.PROJECT_KEYS(projectId), body);

export const revokeProjectKey = (projectId: number, keyId: number): Promise<void> =>
    api.delete(url.PROJECT_KEY_BY_ID(projectId, keyId));
...
export const getTableSchema = (tableName: string) =>
    api.get(url.GRIDBASE_SCHEMA(tableName));

export const getColumnValidation = (tableName: string, columnName: string) =>
    api.get(url.GRIDBASE_COLUMN_VALIDATION(tableName, columnName));

export const setColumnValidation = (
    tableName: string,
    columnName: string,
    body: {
        type: string;
...
) => api.put(url.GRIDBASE_COLUMN_VALIDATION(tableName, columnName), body);
...
//! PROJECT
export interface ProjectListItem { id: number; name: string; description?: string | null; plan: number;tableCount: number; createdAt: string;}
export interface ProjectResponse extends ProjectListItem { updatedAt?: string | null; }
export interface ProjectQuota {used: number;max: number;canCreate: boolean;plan: number; }
export interface CreateProjectBody {  name: string;  description?: string;}
export interface UpdateProjectBody { name?: string; description?: string;}

export const getMyProjects = (): Promise<ProjectListItem[]> => api.get(url.PROJECTS);
export const getProjectQuota = (): Promise<ProjectQuota> =>  api.get(url.PROJECT_QUOTA);
export const getProjectById = (id: number): Promise<ProjectResponse> =>  api.get(url.PROJECT_BY_ID(id));
export const createProject = (body: CreateProjectBody): Promise<ProjectResponse> =>  api.create(url.PROJECTS, body);
export const updateProject = (id: number, body: UpdateProjectBody): Promise<ProjectResponse> => api.put(url.PROJECT_BY_ID(id), body);
export const deleteProject = (id: number): Promise<void> => api.delete(url.PROJECT_BY_ID(id));
export const getProjectOverview = (id: number): Promise<ProjectOverview> => api.get(url.PROJECT_OVERVIEW(id));

//! CorsOrigins
export interface CorsOriginItem { id: number; origin: string; createdAt: string; }
export const getCorsOrigins = (projectId: number): Promise<CorsOriginItem[]> =>
    api.get(url.PROJECT_CORS(projectId));
export const addCorsOrigin = (projectId: number, origin: string): Promise<CorsOriginItem> =>
    api.create(url.PROJECT_CORS(projectId), { origin });
export const removeCorsOrigin = (projectId: number, originId: number): Promise<void> =>
    api.delete(url.PROJECT_CORS_BY_ID(projectId, originId));
