...
using gridbase.Application.Common.Models.Account;
using gridbase.Application.Services.Interfaces;

namespace gridbase.WebApi.Endpoints;
...
public static class AuthEndpoints
{
    public static RouteGroupBuilder MapAuthEndpoints(this IEndpointRouteBuilder routes)
    {
        ...
        // Sadece giriş yapmış birinin oturumu kapatılabilir.
        group.MapPost("/logout", Logout).RequireAuthorization();

        // ── GÜVENLİK DÜZELTMESİ ──
        // block/unblock artık kimlik doğrulaması ister. Ayrıca metot içinde
        // admin/GB rol kontrolü yapılır (aşağıda EnsureAdmin). Önceden token'sız
        // herkes istediği kullanıcıyı bloklayabiliyordu.
    ...
    private static async Task<IResult> AuthenticateByRefreshToken([FromBody] string refreshToken, IAccountService accountService, IConfiguration configuration)
    {
        ...
        // ── GÜVENLİK DÜZELTMESİ ──
        // Bloklu/kilitli kullanıcı, geçerli bir refresh token ile yeni access token
        // ALMAMALI. Aksi halde bloklanan kullanıcı erişmeye devam eder.
        // (IsBlocked / IsLockedOut alan adların farklıysa ona göre düzelt.)
    ...
    private static async Task<IResult> ForgotPassword([FromBody] ForgotPasswordRequest request, IAccountService accountService, IEmailService emailService)
    {
        ...
        if (token != null)
        {
            ...
            await emailService.SendEmailAsync(request.Email, "GridBase - Reset Password",
    ...
    // ── GÜVENLİK DÜZELTMESİ ──
    // block/unblock için admin/GB rol kontrolü. GridBase'deki IsAdmin ile aynı mantık.
    // IUser, mevcut isteğin kullanıcısını (HttpContext claim'lerinden) temsil eder.
    private static bool IsAdmin(IUser currentUser) =>
        string.Equals(currentUser.Role, "Admin", StringComparison.OrdinalIgnoreCase)
        || string.Equals(currentUser.Role, "GB", StringComparison.OrdinalIgnoreCase);
